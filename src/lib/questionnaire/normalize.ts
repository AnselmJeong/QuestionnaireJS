...
function normalizeChoice(choice: ChoiceValue): NormalizedChoice {
...
function normalizeChoices(
  element: RadioQuestion | CheckboxQuestion,
): NormalizedChoice[] {
  const baseChoices = element.choices.map(normalizeChoice);
  const orderedChoices =
    "choicesOrder" in element && element.choicesOrder === "random"
...
function createTextStep(
  ...
  page: QuestionnaireSchema["pages"][number],
  element: TextQuestion,
  panel?: PanelQuestion,
): NormalizedStep {
  const inferredInputType =
    element.maskType === "date" ? "date" : element.inputType ?? "text";
...
function createCheckboxStep(
  form: QuestionnaireSchema,
  page: QuestionnaireSchema["pages"][number],
  element: CheckboxQuestion,
  panel?: PanelQuestion,
): NormalizedStep {
  return {
    id: element.name,
...
function createMatrixSteps(
  form: QuestionnaireSchema,
  page: QuestionnaireSchema["pages"][number],
  element: MatrixQuestion,
  panel?: PanelQuestion,
): NormalizedStep[] {
  const rowsAreRequired = Boolean(
    element.isRequired || element.isAllRowRequired || element.eachRowRequired,
  );
  ...
  switch (element.type) {
    ...
    case "panel":
      return element.elements.flatMap((child) =>
        normalizeElement(form, page, child, element),
      );
    default:
      return [];
  }
...
export function normalizeQuestionnaire(
  ...
  return questionnaire.pages.flatMap((page) =>
    page.elements.flatMap((element) =>
      normalizeElement(questionnaire, page, element),
    ),
  );
}
